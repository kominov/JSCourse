...
function t4() {
    out = [];
    a4.map(item => {
        if (typeof (item) === "number") a4_res.push(item);
    });
    return a4_res;
}
...
function t7() {
    b7_res = b7.filter(item => {
        if ((typeof (item) === "string") && (item.length > 3)) return true;
    });
    return b7_res;
}
...
function t10() {
    b10_res = [];
    let count = 0;
    b10.filter((item, index) => {
        if (b10[index].includes(3)) {
            b10_res[count] = b10[index];
            count++;
        }
    });
    return b10_res;
